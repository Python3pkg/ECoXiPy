...
def enforce_valid_xml_name(value):
    if _xml_name_regex.match(value) is None:
        raise XMLWellFormednessException(
            'The value "{}" is not a valid XML name.'.format(_prepare_value(value)))


def enforce_valid_pi_target(value):
    if _xml_name_regex.match(value) is None or value.lower() == 'xml':
        raise XMLWellFormednessException(
            'The value "{}" is not a valid XML processing instruction target.'.format(_prepare_value(value)))


def enforce_valid_pi_content(value):
    if '?>' in value:
        raise XMLWellFormednessException(
            'The value "{}" is not a valid XML processing instruction content because it contains "?>".'.format(_prepare_value(value)))


def enforce_valid_comment(value):
    if '--' in value:
        raise XMLWellFormednessException(
            'The value "{}" is not a valid XML comment because it contains "--".'.format(_prepare_value(value)))

def enforce_valid_doctype_systemid(value):
    if '"' in value and "'" in value:
        raise XMLWellFormednessException(
            'The value "{}" is not a valid document type system ID.'.format(_prepare_value(value)))


def _doctype_publicid_regex():
    import re
    return re.compile('^[\x20\x0D\x0A]|[a-zA-Z0-9]|[\\-\'\\(\\)\\+,\\./:=\\?;\\!\\*\\#@\\$_%]$')
...
def enforce_valid_doctype_publicid(value):
    if _doctype_publicid_regex.match(value) is None:
        raise XMLWellFormednessException(
            'The value "{}" is not a valid document type public ID.'.format(_prepare_value(value)))
